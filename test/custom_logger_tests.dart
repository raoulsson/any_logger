...
void main() {
  group('Custom Logger Registration', () {
    ...
    test('Logger.defaultLogger should auto-register named loggers (Option 2)', () async {
      ...
      final autoLogger = Logger.defaultLogger([consoleAppender], name: 'AUTO-LOGGER');
    ...
    test('LoggerFactory.createCustomLogger should create and register loggers (Option 3)', () async {
      ...
      final factoryLogger = LoggerFactory.createCustomLogger('FACTORY-LOGGER', [consoleAppender]);
      
      // Verify the logger was created correctly
      expect(factoryLogger.name, equals('FACTORY-LOGGER'));
      expect(factoryLogger.appenders.length, equals(1));
      expect(factoryLogger.appenders.first.getType(), equals('CONSOLE'));
      expect(factoryLogger.appenders.first.level, equals(Level.INFO));
      
      // Test retrieval via factory - should return the SAME instance
      final retrievedLogger = LoggerFactory.getLogger('FACTORY-LOGGER');
      
      // Verify they are the same instance
      expect(identical(factoryLogger, retrievedLogger), isTrue);
      expect(retrievedLogger.name, equals('FACTORY-LOGGER'));
      expect(retrievedLogger.appenders.length, equals(1));
    });
    ...
    test('createCustomLogger should work with file appenders', () async {
      ...
      try {
        ...
        final fileLogger = LoggerFactory.createCustomLogger('FILE-LOGGER', [fileAppender]);
        ...
        final files = testDir.listSync().where((f) => f.path.contains('test_custom_logger')).toList();
    ...
    test('should handle multiple custom loggers with different appenders', () async {
      ...
      final logger1 = LoggerFactory.createCustomLogger('MULTI-1', [consoleAppender]);
    ...
    test('should work with self-debugging enabled', () async {
      ...
      final debugLogger = LoggerFactory.createCustomLogger('DEBUG-LOGGER', [consoleAppender]);
      final retrieved = LoggerFactory.getLogger('DEBUG-LOGGER');
      
      expect(identical(debugLogger, retrieved), isTrue);
    });

    test('createCustomLogger should initialize LoggerFactory if not already initialized', () async {
      ...
      final logger = LoggerFactory.createCustomLogger('INIT-TEST', [consoleAppender]);
